...
import { ComunidadCreateEventModal, type ComunidadEventFormData } from './ComunidadCreateEventModal'
...
export function ComunidadEventsSection() {
  ...
  const [editData, setEditData] = useState<Partial<ComunidadEventFormData> | undefined>(undefined)
  ...
  const loadEvents = useCallback(async () => {
    try {
      const res = await fetch('/api/eventos/comunidad')
      if (!res.ok) return
  ...
  async function createEvent(data: ComunidadEventFormData) {
    const supabase = createClient()
    const {
      data: { session },
  ...
  async function updateEvent(data: ComunidadEventFormData) {
    ...
    await fetch(`/api/eventos/comunidad/${editingEvent.id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${session.access_token}`,
